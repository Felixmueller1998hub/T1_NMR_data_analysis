...
def fit_Li6_LITFSI_with_Argyrodite(measurements, plot = False):
    # Model for PEO Peak
    mod_PEO = LorentzianModel()
    pars = mod_PEO.guess(measurements['intensity'], x=measurements['chemical shift'])
    pars['amplitude'].set(value=10**6, min = 10**4, max = 10**13)
...
def make_dataframe(T1_dataframe, chemical_shift):
    ...
    for i in range (19, -1, -1):
        ...
        Arg_fit[i] = lorentzian(chemical_shift, Arg_center, Arg_amplitude, Arg_sigma)
        PEO_fit[i] = lorentzian(chemical_shift, PEO_center, PEO_amplitude, PEO_sigma)
...
T1data = pd.DataFrame(columns=['FWHM_PEO', 'FWHM_Argyrodite', 'Height_PEO', 'Height_Argyrodite', 'error_heightPEO', 'error_heightArg', 'error_fwhmPEO', 'error_fwhmArg', 'center_PEO', 'center_Argyrodite', 'amplitude_PEO', 'amplitude_Argyrodite'])
...
for i in range(len(vd_list), 0, -1):
    ...
    height_PEO = amplitude_PEO/(fitted_params['sigma']*np.pi)
    ...
    error_heightArg = height_Argyrodite * np.sqrt((std_errors[5]/amplitude_Argyrodite)**2 + (std_errors[7]/fitted_params['Argyrodite_sigma'])**2 )
    error_fwhmPEO = 2*std_errors[2]
    error_fwhmArg = 2*std_errors[7]
    
    # Save parameters to DataFrame using loc
    T1data.loc[i - 1] = [fwhm_PEO, fwhm_Argyrodite, height_PEO, height_Argyrodite, error_heightPEO, error_heightArg, error_fwhmPEO, error_fwhmArg, center_PEO, center_Argyrodite, amplitude_PEO, amplitude_Argyrodite]
    ...
    plt.plot(measurements['chemical shift'], lorentzian(measurements['chemical shift'], fitted_params['center'], fitted_params['amplitude'], fitted_params['sigma']),'--',label = 'PEO')
    plt.plot(measurements['chemical shift'], lorentzian(measurements['chemical shift'], fitted_params['Argyrodite_center'], fitted_params['Argyrodite_amplitude'], fitted_params['Argyrodite_sigma']),'--',label = 'Argyrodite')
