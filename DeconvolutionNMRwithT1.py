...
for i in range(len(vd_list), 0, -1):
    ...
    height_PEO = (1-fitted_params['alpha'])*(amplitude_PEO/(fitted_params['sigma']*np.pi))+ fitted_params['alpha']*(amplitude_PEO/(fitted_params['sigma']*np.sqrt(2*np.pi)))
    ...
    error_fwhmPEO = 2+(fitted_params['alpha']*0.3548)*std_errors[2]
